...
def argmax_theta_random(batch, model, n_samp):
    ...
    rotated = [torchvision.transforms.functional.rotate(x, theta) for theta in angles]
    rotated = torch.cat(rotated, 0)
    losses = F.mse_loss(rotated, model(rotated), reduction='none')
    losses = torch.reshape(losses, (n_samp, batch.shape[0], batch.shape[1], batch.shape[2], batch.shape[3]))
    losses = torch.mean(losses, dim=(1,2,3,4)).tolist()
...
#     iterator = loopy(train_loader)
    ...
    plt.savefig(f'../plots_rotations_random/fixed_epoch{epoch}.png')
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for idx, data in enumerate(train_loader):
            ...
            if epoch < 0:
                ...
                model.eval()
                with torch.no_grad():
                    theta = argmax_theta_random(x, model, n_samp=8)
#                 print(idx, theta)
#                 sys.stdout.flush()
                model.train()
                theta_save = theta
...
#     save_name = '../trained_no_robust2/ae_c_d{}L{}.pt'.format(latent_dim, L)
    save_name = f'../trained_robust_rotations_random/ae_c_d{latent_dim}L{L}rotated.pt'
