...
def obj_rot(theta, img_iterator, model):
    ...
    x_hat = model(x, 1)
    loss = -32*32*F.mse_loss(x, x_hat).item()
    return loss
...
def argmax_theta(train_loader, model):
    ...
    theta_max = scipy.optimize.minimize_scalar(obj, bounds=[-180, 180], method='bounded', options={'maxiter': 40}).x
    return theta_max
...
#     save_name = '../trained_robust_WassDRO/ae_c_d{}L{}.pt'.format(latent_dim, L)
    save_name = f'../trained_rotations/ae_c_d{d_pair[0]}-{d_pair[1]}L{L}rotated.pt'
